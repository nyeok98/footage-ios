...
extension HomeViewController {
    func trackMapView() {
        ...
        for journey in DateManager.loadFromRealm(rangeOf: today) { // draw previous routes from today
            mainMap.drawPolyline(from: Array(journey.footsteps))
        }
...
// MARK: - Example View
    ...
    @IBAction func questionCircleReleased(_ sender: UIButton) {
        exampleImageView.isHidden = true
    }
...
extension HomeViewController: ButtonViewContainer {
    func mainColorDidChange(hex: String) {
        mainMap.tintColor = UIColor(hex: hex)
    }
}
